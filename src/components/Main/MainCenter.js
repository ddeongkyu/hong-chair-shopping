...
function MainCenter() {
  ...
  const filterBtn = ["All", "Dark", "Fabric", "Colored"];
  const dispatch = useDispatch();
  const { filter, shoppingCart, isLoading, products } = useSelector((state) => {
    return state.product;
  });
  const filteredProducts = useGetProductbyKeyWord(filter);
